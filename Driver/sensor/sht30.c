...
//-- Global variables ----------------------------------------------------------
static uint8_t _i2cWriteHeader;
static uint8_t _i2cReadHeader;
static float fTemp;
static float fHumi;
...
//==============================================================================
	...
	I2c_Init(); // init I2C
	SHT3X_SetI2cAdr(SHT30_ADDRESS);
	error = SHT3x_ReadSerialNumber(&serialNumber);
	error += SHT3X_GetTempAndHumi(&fTemp, &fHumi, REPEATAB_HIGH, MODE_POLLING, 255);
	error += SHT3X_StartPeriodicMeasurment(REPEATAB_HIGH, FREQUENCY_4HZ);
	if (error == NO_ERROR)
		return SUCCESS;
	else
		return ERROR;
...
//==============================================================================
etError SHT3X_Read2BytesAndCrc(uint16_t *data, etI2cAck finaleAckNack, uint8_t timeout)
{
	etError error;    // error code
...
uint16_t SHT3X_CalcRawTemperature(float temperature)
{
	...
	return (uint16_t)((temperature + 45) / 175 * 65535);
}
...
uint16_t SHT3X_CalcRawHumidity(float humidity)
{
	...
	return (uint16_t)(humidity / 100 * 65535);
}
...
{
	etError error;
	error = SHT3X_ReadMeasurementBuffer(&fTemp, &fHumi);
	if (error == NO_ERROR)
	{
		*wTemp = (int16_t)(fTemp * 10);
		*wHumi = (uint16_t)(fHumi * 10);
		return SUCCESS;
	}
	return ERROR;
}